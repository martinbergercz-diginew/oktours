...
```
Browser ──HTTPS basic-auth──▶ Caddy ──reverse-proxy──▶ admin-service (this) :3000
                                          │
                       ┌──────────────────┼────────────────────┐
                       ▼                  ▼                    ▼
...
- `/srv/oktours-repo/` — checked-out monorepo. Claude edits files under `ok-tours/`.
...
In dry-run mode:
- `REPO_PATH` defaults to the monorepo root (this checkout). Claude edits real files in `ok-tours/`.
...
# Clone the monorepo somewhere Claude can edit:
git clone https://github.com/martinbergercz-diginew/prototypes.git /srv/oktours-repo
